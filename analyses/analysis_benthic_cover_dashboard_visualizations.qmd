---
title: "WCS MERMAID Explore Visualizations"
subtitle: "Benthic Cover"
...
This code provides examples of visualizations shown on the MERMAID dashboard ([https://dashboard.datamermaid.org](https://dashboard.datamermaid.org/)) that use data from one or more of the benthic protocols that collect information about the % cover of benthic categories (PIT, LIT, Photo Quadrat, and Bleaching surveys).

## Getting summary sample event data from MERMAID

The following code downloads summary sample event data from MERMAID ([https://datamermaid.org](https://datamermaid.org/)), using the mermaidr package (documentation can be found at <https://data-mermaid.github.io/mermaidr/>). The summary sample events contain all surveys (i.e. sample events) that have permissions of "public summary" or "public".
...
hardCoralAggHist <-
  ...
  style(hovertemplate = "Bin: %{x}% cover<br>%{y} surveys<extra></extra>") 
...
benthicPitSingleSeTBL <- benthicAvgTBL %>% 
  ...
  filter(benthicpit_sample_unit_count > 0) #Get only those with benthic PIT protocol
